 ...
 * @author
 */
public class Socket3Receiver implements Runnable{
    static DatagramSocket receiving_socket;
    private AudioPlayer player;

    public Socket3Receiver(){
        try {
...
//        String dataString;
        ...
        while (GlobalVariables.Socket3ReceiveRun){
...
//                // TEXT END
                ...
                receiving_socket.receive(packet);
                    ...
                    UI.Socket3StopReceive.setEnabled(false);
                     UI.log2.setText("Timed Out");
                    GlobalVariables.Socket3ReceiveRun = false;
                    break;
                }
                
                byte[] headerBufferTest = new byte[4];
                System.arraycopy(buffer, 0, headerBufferTest, 0, 4);
                String header1StringTest = new String(headerBufferTest);
                if (header1StringTest.endsWith("0000")){
                    UI.Socket1StartReceive.setEnabled(true);
                    UI.Socket2StartReceive.setEnabled(true);
                    UI.Socket3StartReceive.setEnabled(true);
                    UI.Socket4StartReceive.setEnabled(true);
